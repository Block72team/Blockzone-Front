...
function handlePostResult(resultData) {
    ...
    for (let i = 0; i < resultData.length; i++) {
        ...
        let rowHTML = '';
        rowHTML += '<h2 class="mt-0">' + resultData[i]['title'] + '</h2>';
        rowHTML += '<img class="col-11" src="' + resultData[i]['thumb_url'] + '" alt="Generic placeholder image">';
        ...
        rowHTML += '<h5>Share This:</h5><br>';
        rowHTML += '<a class="btn btn-social-icon btn-twitter" style="margin: 5px" href="https://twitter.com/intent/tweet/?url=blockzone.com/post?id=' + resultData[i]['id'] + '"><span class="fab fa-twitter"></span></a>';
        rowHTML += '<a class="btn btn-social-icon btn-facebook" style="margin: 5px" href="https://facebook.com/sharer/sharer.php?u=blockzone.com/post?id=' + resultData[i]['id'] + '"><span class="fab fa-facebook"></span></a>';
        rowHTML += '<a class="btn btn-social-icon btn-linkedin" style="margin: 5px" href="https://www.linkedin.com/shareArticle?mini=true&amp;url=blockzone.com/post?id=' + resultData[i]['id'] + '"><span class="fab fa-linkedin"></span></a>';
        rowHTML += '</div></div>';
        rowHTML += '<div class="col-8">';
        rowHTML += '<p>' + resultData[i]['content'] + '</p>';
        rowHTML += '<center><button id="like" class="btn btn-outline-secondary" type="button" onclick="like()"><i id="like-icon" class="fas fa-thumbs-up" style="margin-right: 10px;"></i>Like this!</button></center>';
        rowHTML += '</div></div><hr>';
